 ...
 */
object SparkInserHivetHudiExample {
  def main(args: Array[String]): Unit = {
    ...
    newdf.write.format("hudi")
      ...
      // 指定Hive映射的表名称
      .option(DataSourceWriteOptions.HIVE_TABLE.key(), "infos1")
      // Hive表映射对的分区字段
      .option(DataSourceWriteOptions.HIVE_PARTITION_FIELDS.key(), "loc")
