...
public class FlinkKafkaCompany2DorisDemo {
    ...
    public static void main(String[] args) {
        ...
        SingleOutputStreamOperator<JSONObject> dataFilter = streamOperator.filter(new FilterFunction<JSONObject>() {
            ...
            public boolean filter(JSONObject value) {
                ...
                return pubTime > (System.currentTimeMillis() - DAY_400) && pubTime < (System.currentTimeMillis() + DAY_400 / 400 * 3);
            }
        });
        SingleOutputStreamOperator<String> operator = dataFilter.map(new MapFunction<JSONObject, String>() {
            ...
            public String map(JSONObject value) {
                ...
                result.put("word_cloud",new JSONArray());
                result.put("subjects",new JSONArray());
        ...
        dorisOp.setFenodes("192.168.67.131:8030")
                .setTableIdentifier("example_db.mf_company_ods_958_2")
                .setUsername("root")
                .setPassword("");
    ...
    private static void removeKey(JSONObject value) {
        ...
        value.remove("sysSentimentTag");
        value.remove("originalContentSimHash");
        value.remove("createDay");
        value.remove("crawlDay");
        value.remove("pubDate");
        value.remove("crawlDate");
        value.remove("titleLength");
        value.remove("sysAbstract");
        value.remove("createDate");
    }
